(function(global){
 ...
 function mount({host,state,act}){
  ...
  function render(){const s=state();host.hidden=!s.visible;if(host.hidden)return;current=next(s);
   const html=`<div class="rf-search-flow-top"><div><span class="rf-search-flow-eyebrow">${esc(s.job?.title||'Your next search')}</span><p>${esc(current.text)}</p></div><div class="rf-actions"><button type="button" class="rf-btn primary" data-search-action="next">${esc(current.label)} →</button>${s.job?'<button type="button" class="rf-linkbtn" data-search-action="start">Start another search</button>':''}</div></div><ol aria-label="Search workflow">${['Set up job','Upload resumes','Review candidates','Prepare submittal'].map((label,i)=>`<li ${i===current.step?'aria-current="step"':''}><span>${i+1}</span>${label}</li>`).join('')}</ol>`;
 ...
 const api={next,mount};if(typeof module!=='undefined')module.exports=api;global.AncalagonSearchFlow=api;
