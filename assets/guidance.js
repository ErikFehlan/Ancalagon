  ...
  function mount(root,api){
    ...
    function render(){
      ...
      root.querySelectorAll('[data-guidance-tip]').forEach(slot=>{
        ...
        const html=`<div class="rf-guidance ${expanded?'is-open':''}"><div class="rf-guidance-head"><button type="button" class="rf-linkbtn" data-guidance-open="${key}" aria-expanded="${expanded}" aria-controls="${bodyId}"><span class="rf-help-mark" aria-hidden="true">?</span>${expanded?tip.title:'Show guidance: '+tip.title}</button>${expanded?`<button type="button" class="rf-linkbtn" data-guidance-dismiss="${key}" ${v.busy?'disabled':''}>${v.loaded?'Got it':'Close'}</button>`:''}</div><div id="${bodyId}" ${expanded?'':'hidden'}><p>${tip.text}</p><details><summary>More context</summary><p>${tip.more}</p></details></div>${v.problem?`<p class="rf-guidance-status" role="status">${v.problem} <button type="button" class="rf-linkbtn" data-guidance-retry ${v.busy?'disabled':''}>Retry</button></p>`:''}</div>`;
