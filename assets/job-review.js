  ...
  function create(api){
    ...
    function body(task,c,inline=false){
      ...
      return `<p class="rf-reevaluation-score"><span>${Number(c.managerScore).toFixed(1)}</span><span>→</span><strong>${Number(result.manager_score).toFixed(1)} / 10</strong></p>
        ...
        <p>${escape(result.manager_reason||result.summary)}</p><details><summary>Why this assessment? View evidence</summary><p>${escape(result.jd_reason)}</p><ul>${(result.evidence_ids||[]).map(id=>{const source=api.context(c).sources.find(s=>s.id===id);return `<li>${source?`<strong>${escape(source.kind)}:</strong> ${escape(source.text.slice(0,800))}`:'Source changed; refresh before reviewing.'}</li>`;}).join('')}</ul></details>
        <p class="rf-evidence-caution">Missing evidence is a question to verify, not proof of a missing skill.</p>${result.questions?.length?`<div class="rf-review-questions"><h4>What to verify</h4><ul>${result.questions.map(q=>`<li>${escape(q)}</li>`).join('')}</ul></div>`:''}<details class="rf-review-explanation"><summary>What happens when I approve?</summary><p>Saves the proposed JD Fit and Manager Fit scores, recommendation, and supporting assessment for this candidate. Their pipeline stage and other candidates stay unchanged.</p><p>Keep current assessment dismisses this proposal and retains the current scores. Your notes remain saved. Approve &amp; next also opens the next ready review.</p></details>
