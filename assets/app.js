    (()=>{
      ...
      const adminTools=window.AncalagonAdminTools.create({
        ...
        toast:showToast,saveFile:downloadBlob
      });
      ...
      function setAdminAccess(allowed){
        ...
        if(!adminAccess){
          ...
          for(const id of ['adminAccounts','adminActive7','adminActive30','adminSessions','adminEvents'])root.querySelector('#'+id).textContent='—';
      ...
      async function loadAdminUsage(){
        ...
        try{
          ...
          if(request===adminRequest)setAdminAccess(false);
...
function renderJobs(){
  ...
  list.querySelectorAll('[data-reopen-job]').forEach(b=>b.addEventListener('click',()=>reopenJob(b.dataset.reopenJob)));
      ...
      function showPage(name,{navigationToken}={}){
        ...
        if(!jobs.length&&!['home','jobs','job-picker','learn','backend','admin-tools','admin-usage'].includes(name))name='home';
        ...
        if(name==='admin-tools')void adminTools.open();
