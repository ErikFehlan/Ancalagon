...
function renderJobs(){
  ...
  list.querySelectorAll('[data-reopen-job]').forEach(b=>b.addEventListener('click',()=>reopenJob(b.dataset.reopenJob)));
      ...
      window.addEventListener('ancalagon:auth-cleared',()=>{workspaceGeneration++;dataReady=false;adminRequest++;setAdminAccess(false);settings?.clear();jobs.splice(0);candidates.splice(0);feedback.splice(0);interviewOutcomes.splice(0);activeJobId=null;home?.dispose();tutorial?.dispose();root.querySelector('#personalUsage').textContent='';});
