  ...
  saveState();renderJobs();recalibrateAll();renderFeedback();renderOutcomes();showToast('Candidate and linked records deleted.');
      ...
      const intake=window.AncalagonIntake.create({
        root,beforeReview:prepareAssessmentReview,next:advanceAfterReview,reviewed:()=>{void guidance?.complete('assessment');void guidance?.complete('approval');},job:id=>id?jobs.find(j=>j.id===id):activeJob(),candidates:()=>candidates,
