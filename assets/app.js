    (()=>{
      ...
      async function loadAdminUsage(){
        ...
        try{
          ...
          root.querySelector('#qualityLab').classList.remove('rf-hidden');
          ...
          root.querySelector('#qualityLab').classList.add('rf-hidden');
      ...
      function renderGlobalContext(){
        const select=root.querySelector('#globalJobSelect'),options=jobs.filter(j=>j.status!=='closed'||j.id===activeJobId).map(j=>`<option value="${escapeHTML(j.id)}">${escapeHTML(j.title)}${j.status==='closed'?' · Closed':''}${j.client?' · '+escapeHTML(j.client):''}</option>`).join('');
  ...
  saveState();renderJobs();recalibrateAll();renderFeedback();renderOutcomes();showToast('Candidate and linked records deleted.');
      ...
      function renderPipeline(){const stages=['Sourced','Screened','Submitted','Interviewing','Offer','Hired','Rejected','Withdrew'];const board=root.querySelector('#pipelineBoard');board.innerHTML=stages.map(s=>`<div class="rf-stage"><h4>${s} · ${activeCandidates().filter(c=>c.stage===s).length}</h4>${activeCandidates().filter(c=>c.stage===s).map(c=>`<div class="rf-stageitem" data-pipeline-name="${c.name.replace(/"/g,'&quot;')}"><strong>${escapeHTML(c.short)}${c.stage==='Hired'?' 🙂':''}</strong><div class="rf-sub">${window.AncalagonIntake.pending(c)?'Awaiting assessment review':c.managerScore.toFixed(1)+'/10 · '+escapeHTML(c.rec)}</div></div>`).join('')||'<div class="rf-sub">No candidates</div>'}</div>`).join('');board.querySelectorAll('[data-pipeline-name]').forEach(x=>x.addEventListener('click',()=>openDetail(x.dataset.pipelineName)))}
      ...
      function renderCompareSelectors(){const opts='<option value="">Select a candidate…</option>'+ranked().map(c=>`<option value="${c.name.replace(/"/g,'&quot;')}">${escapeHTML(c.short)} · ${c.managerScore.toFixed(1)}/10</option>`).join('');['compare1','compare2','compare3','compare4'].forEach(id=>{const select=root.querySelector('#'+id);select.innerHTML=opts;select.onchange=updateCompareStatus});updateCompareStatus()}
      ...
      function bindFeedbackInterpretations(list){
        ...
        list.querySelectorAll('[data-correct-interpretation]').forEach(b=>b.addEventListener('click',()=>{const f=feedback[Number(b.dataset.correctInterpretation)],text=b.parentElement.querySelector('textarea').value.trim();if(!f||!text)return;f.interpretation={text,source:'recruiter',updatedAt:Date.now()};f.updatedAt=Date.now();saveState();renderFeedback();candidateAutomation.request(candidateForRef(f.candidateId,f.jobId));showToast('Clarification saved. Preparing an updated assessment.')}));
      }
      ...
      function refreshIntakeCandidate(c,detailOnly=false){
        ...
        root.querySelector('#detailName').textContent=c.short;root.querySelector('#detailRole').textContent=c.role;
        root.querySelector('#detailSignal').textContent=c.signal;
      ...
      const jobReview=window.AncalagonJobReview.create({
        ...
        review:(id,revision,decision)=>dataService.reviewJobReassessment(id,revision,decision,stateSnapshot()),
...
function renderJobs(){
  ...
  list.querySelectorAll('[data-reopen-job]').forEach(b=>b.addEventListener('click',()=>reopenJob(b.dataset.reopenJob)));
      ...
      function showPage(name,{navigationToken}={}){
        ...
        focusUI.show(name,activeJob(),candidate,navigation);home?.remember(name,activeJobId,candidate?.id||null);
      }
      ...
      function renderInitialState(){root.querySelector('#learnWelcome').classList.toggle('rf-hidden',jobs.length>0);renderJobs();renderJobPicker();if(jobs.length){loadActiveJobWeights();recalibrateAll();renderFeedback();renderOutcomes();}showPage('home')}
