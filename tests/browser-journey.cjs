...
await page.addInitScript(()=>{window.testSaved=null;window.AncalagonData={create:()=>({load:async()=>window.reloadFixture||({jobs:[],candidates:[],feedback:[],interviewOutcomes:[]}),schedule:(s,e,status)=>{window.testSaved=JSON.parse(JSON.stringify(s));status('saved')},flush:async s=>{window.testSaved=s},trackEvent:async()=>{},loadAdminAnalytics:async()=>{throw Error('not admin')}})};window.ancalagonAuth={session:{user:{id:'test'},access_token:'test-token'},workspace:{id:'test'}}});
