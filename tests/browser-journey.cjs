...
await page.addInitScript(()=>{window.testSaved=null;window.AncalagonData={create:()=>({load:async()=>window.reloadFixture||({jobs:[],candidates:[],feedback:[],interviewOutcomes:[]}),schedule:(s,e,status)=>{window.testSaved=JSON.parse(JSON.stringify(s));status('saved')},flush:async s=>{const copy=JSON.parse(JSON.stringify(s));if(window.testDelayNextFlush){window.testDelayNextFlush=false;await new Promise(resolve=>{window.releaseFlush=resolve})}if(window.testFailNextFlush){window.testFailNextFlush=false;throw Error('simulated save failure')}window.testSaved=copy},trackEvent:async()=>{},loadAdminAnalytics:async()=>{if(window.testAdmin)return{totals:{},users:[]};throw Error('not admin')}})};window.ancalagonAuth={session:{user:{id:'test'},access_token:'test-token'},workspace:{id:'test'}}});
...
await page.locator('#feedbackList .rf-feedback-interpretation').waitFor();
assert.match(await page.locator('#feedbackList').textContent(),/Interpretation ready · saving/);
assert.equal(await page.evaluate(()=>window.testSaved.feedback[0].interpretation),undefined);
await page.evaluate(()=>window.releaseFlush());
...
await page.locator('#feedbackSubmitBtn').click();
await page.locator('#feedbackList .rf-feedback-interpretation').nth(1).waitFor();
...
await page.locator('#workspaceEvaluation [data-evaluation-action="apply"]').waitFor();
assert.match(await page.locator('#workspaceEvaluation').textContent(),/Specific ownership example/);
assert.equal(await page.evaluate(()=>window.testSaved.jobs[0].patternAnalysis||null),null,'automatic assessment needs no pattern analysis');
const proposalBeforeApproval=await page.evaluate(()=>window.testSaved.candidates[0].feedbackEvaluation.proposal);
assert.notEqual(proposalBeforeApproval.currentScore,proposalBeforeApproval.proposedScore);
...
assert.ok(await page.evaluate(()=>window.testSaved.feedback[2].interpretation),'retained interpretation persisted on retry/save');
await page.setViewportSize({width:390,height:844});
assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth+1),true,JSON.stringify(await page.evaluate(()=>[...document.querySelectorAll('#rf-app *')].filter(e=>e.getBoundingClientRect().right>innerWidth+1&&getComputedStyle(e).position!=='fixed').slice(0,12).map(e=>({tag:e.tagName,id:e.id,cls:e.className,parent:e.parentElement.className,text:e.tagName==='SPAN'?e.textContent:'',right:e.getBoundingClientRect().right,width:e.getBoundingClientRect().width})))));
await page.setViewportSize({width:1280,height:900});
await page.locator('#workspaceEvaluation [data-evaluation-action="apply"]').waitFor();
const pendingProposal=await page.evaluate(()=>window.testSaved.candidates[0].feedbackEvaluation.proposal);
assert.equal(pendingProposal.status,'pending');
assert.equal(await page.inputValue('#submissionDraft'),'Recruiter edited summary: testing evidence recorded; automation ownership needs confirmation.','async assessment preserves the draft');
...
await page.locator('.rf-nav [data-page="candidates"]').click();
await page.locator('[data-candidate-id]').first().click();
await page.locator('#workspaceEvaluation [data-evaluation-action="ignore"]').click();
assert.equal(await page.evaluate(()=>window.testSaved.candidates[0].managerScore),9);
assert.equal(await page.evaluate(()=>window.testSaved.candidates[0].feedbackEvaluation.status),'ignored');
...
await page.locator('#page-dashboard.active').waitFor();
assert.equal(heldAssessment.payload.job.title,'Reliability test');
assert.equal(heldAssessment.payload.evaluation_context.job_id,saved.jobs[0].id);
heldAssessment.resolve();
await page.waitForFunction(()=>window.testSaved.candidates[0].feedbackEvaluation.status==='pending');
assert.equal(await page.evaluate(()=>window.testSaved.activeJobId),await page.evaluate(()=>window.testSaved.jobs[1].id));
assert.equal(await page.evaluate(()=>window.testSaved.candidates[0].jobId),saved.jobs[0].id);
