...
test('runner sends exactly two ordered requests per case and never transmits rubrics',async()=>{
 ...
 assert.equal(calls.length,suite.cases.length*2);assert.equal(report.status,'completed');assert.equal(report.results.length,suite.cases.length);
...
test('failed first stage never fabricates a follow-up; other cases can finish',async()=>{
 ...
 assert.equal(calls,suite.cases.length*2-1);assert.equal(report.status,'completed_with_errors');assert.equal(report.results[0].stages.length,0);assert.match(report.results[0].error,/unavailable/);
});
...
test('baseline comparison rejects incompatible or duplicated cases',async()=>{
 const r=await suite.run(async()=>output(8));assert.equal(suite.compare(r,r).length,suite.cases.length);
