...
Deno.test('learning models are workspace scoped, feedback only, server selected, and fail back to base',async()=>{
 ...
 let allowed=true,lookupFail=false,modelFail=false,lookups=0;const models:string[]=[];
 ...
 try{
  ...
  modelFail=true;const before=models.length;assert((await handleAuthenticatedAnalysis(request())).ok&&models.length===before+2&&models.at(-1)===base,'invalid custom output did not fall back');modelFail=false;
