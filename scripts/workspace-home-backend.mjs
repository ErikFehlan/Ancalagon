...
const response=await fetch('https://api.supabase.com/v1/projects/'+ref+'/database/query',{
 ...
 body:JSON.stringify({query:(await Promise.all(['20260914210000_workspace_home.sql','20260915110000_guided_tutorial.sql'].map(name=>readFile('supabase/migrations/'+name,'utf8')))).join('\n')}),signal:AbortSignal.timeout(60000)});
if(!response.ok)throw Error('Workspace Home migration failed ('+response.status+'). No query result was logged.');
console.log('Personal workspace starting points and tutorial progress are ready.');
